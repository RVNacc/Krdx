...
import { GlobalKardex } from "./GlobalKardex";
import {
  AlertTriangle,
  Layers,
  HelpCircle,
...
export function ReportView({
    ...
    import("../lib/utils").then(({ exportToCsv }) => {
      ...
      history.forEach((e) => {
        ...
        rows.push([
          ...
          e.unit || "-",
          String(isIncoming ? e.quantity : ""),
          String(isIncoming ? e.unitPrice : ""),
          String(isIncoming ? e.totalPrice : ""),
          String(isOutgoing ? e.quantity : ""),
          String(isOutgoing ? e.unitPrice : ""),
          String(isOutgoing ? e.totalPrice : ""),
          String(isOutgoing && e.type === "SALE" ? (e.vat || 0) : ""),
          String(isOutgoing && e.type === "SALE" ? (e.totalPrice + (e.vat || 0)) : ""),
          String(isOutgoing && e.type === "SALE" && e.cogs ? e.cogs : ""),
          String(e.balanceQuantity),
          String(e.balanceTotalCost),
          String(e.averageUnitCost),
        ]);
      });
  ...
  return (
    <div className="w-full max-w-7xl mx-auto flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 p-6 bg-white rounded-2xl shadow-sm border border-gray-150/80">
        ...
        <div className="lg:col-span-4 flex flex-col justify-center">
          ...
          <p className="text-xs text-gray-500 mt-1.5 leading-relaxed">
            مدیریت موجودی‌های منفی، تعدیل و شبیه‌سازی نرخ‌ها، دسته‌بندی مالیاتی،
            کنترل تفصیل و تطبیق سودآوری.
          </p>
        </div>
        ...
        <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <select
              ...
              onChange={(e) =>
                onStateChange({ selectedTafsil: e.target.value })
              }
              className="p-2.5 border border-gray-200 rounded-xl bg-gray-50 text-xs font-bold text-gray-800 focus:outline-none focus:border-emerald-500 focus:bg-white"
            >
              <option value="__ALL__">
                همه مشتریان و تفصیل‌ها (بدون فیلتر)
              </option>
              {allTafsils.map((t) => (
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <select
              ...
              onChange={(e) =>
                onStateChange({ negativeStockMode: e.target.value as any })
              }
              className="p-2.5 border border-gray-200 rounded-xl bg-gray-50 text-xs font-bold text-gray-800 focus:outline-none focus:border-emerald-500 focus:bg-white"
            >
              <option value="ALLOW">آزاد (بدون اعمال محدودیت)</option>
              <option value="ZERO_OUT">
                حذف منفی (کپ سقف خروجی به اندازه موجودی روز)
              </option>
              <option value="ADJUST_INITIAL">
                تعدیل هوشمند (افزایش خودکار اول دوره جهت برابری)
              </option>
            </select>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-6">
          <div className="flex items-center gap-1.5">
            ...
            {negativeStockMode === "ALLOW" && (
              <span className="text-amber-600 bg-amber-50 px-2 py-0.5 rounded border border-amber-100 font-bold">
                بدون کنترل (آزاد)
              </span>
            )}
            {negativeStockMode === "ZERO_OUT" && (
              <span className="text-sky-600 bg-sky-50 px-2 py-0.5 rounded border border-sky-100 font-bold">
                باقیمانده صفر (پوشش خروجی)
              </span>
            )}
            {negativeStockMode === "ADJUST_INITIAL" && (
              <span className="text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-100 font-bold">
                تعدیل هوشمند اول دوره (ترمیم خودکار)
              </span>
            )}
          </div>
        </div>
              ...
              import("../lib/utils").then(({ exportToCsv }) => {
                ...
                summaries.forEach((s) => {
                  rows.push([
                    ...
                    String(s.purchasedValue),
                    String(s.soldQuantity),
                    String(s.salesRevenue),
                    String(s.endingQuantity),
                    String(s.endingValue),
                    String(s.cogs),
                    String(s.grossProfit),
                    String(s.averageUnitCost),
                  ]);
                });
      ...
      <div className="flex border-b border-gray-200">
        ...
        <button
          onClick={() => setActiveTab("GLOBAL_KARDEX")}
          className={`px-6 py-3 font-bold text-sm transition-colors ${activeTab === "GLOBAL_KARDEX" ? "border-b-2 border-indigo-600 text-indigo-800 bg-white/40" : "text-gray-500 hover:text-gray-850"}`}
        >
          کاردکس کل (تجمیعی)
        </button>
        <button
          onClick={() => setActiveTab("OPTIMIZER")}
          className={`px-6 py-3 font-bold text-sm transition-colors ${activeTab === "OPTIMIZER" ? "border-b-2 border-purple-600 text-purple-800 bg-white/40" : "text-gray-500 hover:text-gray-850"}`}
        >
            ...
            <table className="w-full text-sm text-right">
              <thead className="bg-gray-50 border-b border-gray-200 text-gray-600 font-bold">
                <tr>
                  ...
                  <th className="p-4 text-xs font-black bg-amber-50/40">
                    بهای فروش رفته (COGS)
                  </th>
                  <th className="p-4 text-xs font-black bg-emerald-50/40">
                    حاصل سود / زیان
                  </th>
                ...
                {paginatedHistory.length === 0 ? (
                  ...
                  paginatedHistory.map((entry, idx) => {
                    ...
                    return (
                      ...
                      >
                        <td className="p-4">
                          <span className="text-gray-450 block text-[9px] font-mono">
                        ...
                        <td
                          className="p-4 font-semibold text-gray-600 truncate max-w-[120px]"
                          title={entry.tafsil}
                        >
                          {entry.tafsil || "—"}
                        </td>
  ...
  {/* Unit Price Input */}
                       <td className="p-4 font-mono font-bold text-indigo-700 bg-indigo-50/20">
                        ...
                        <td
                          className={`p-4 font-mono font-bold bg-emerald-50/10 text-right ${entry.profit > 0 ? "text-emerald-700" : entry.profit < 0 ? "text-rose-600" : "text-gray-400"}`}
                        >
                          {entry.profit !== 0
              ...
              {totalPages > 1 && (
                <div className="flex items-center gap-4">
                  <span className="text-xs text-gray-500 font-medium">
                    ...
                    {Math.min(currentPage * rowsPerPage, history.length)} از{" "}
                    {history.length}
                  </span>
            ...
            {hasAnyAdjustments && (
              <div className="mt-8 p-4 bg-amber-50 rounded-xl border border-amber-200/50 flex flex-col gap-2 text-xs text-amber-900">
                <strong className="font-bold flex items-center gap-1">
                  <AlertTriangle className="w-4 h-4 text-amber-500" /> توجه: ریز
                  مبالغ بر روی سود اعمال شده‌اند.
                </strong>
                تعدیلات دستی نرخ در فایلهای کاردکس فعال است. ارقام سود ناخالص
                بالا بازتاب‌دهنده قیمت‌های هدف جدید هستند. برای بازگشت به فایل
                اصلی، گزینه حذف تمام نرخ‌های جایگزین را در نوار وضعیت کلیک کنید.
              </div>
            )}
          </div>
        )}
          ...
          <div className="p-8">
            <div className="mb-6">
              <h3 className="text-lg font-bold text-gray-800">
                گزارش تراز و عوارض بر اساس گروه درصد عوارض (مالیات سطری)
              </h3>
              <p className="text-xs text-gray-500 mt-1 leading-relaxed">
                در این بخش، محصولات و فاکتورهای فروش بر اساس کدهای ارزش افزوده و
                مالیاتی گروه بندی شده‌اند. این فرآیند بر مبنای ستون مالیاتی اکسل
                بارگذاری شده کاردکس تنظیم می‌شود.
              </p>
            </div>
                ...
                <tbody className="divide-y divide-gray-100 text-xs">
                  {taxGroups.length === 0 ? (
                    ...
                    taxGroups.map((group, idx) => {
                      const margin =
                        group.totalSales > 0
                          ? (group.totalProfit / group.totalSales) * 100
                          : 0;
                      return (
                        <tr
                          key={idx}
                        ...
                        >
                          ...
                          <td className="p-4 font-mono font-semibold">
                            {group.itemsCount} کالا
                          </td>
                          <td className="p-4 font-mono text-gray-500">
                            {group.recordsCount} فاکتور
                          </td>
                          <td className="p-4 font-mono font-bold">
                            {formatCurrency(group.totalSales)} ریال
                          </td>
                          <td className="p-4 font-mono text-left text-gray-600 font-semibold">
                            {formatCurrency(group.totalVat)} ریال
                          </td>
                          <td className="p-4 font-mono text-gray-500">
                            {formatCurrency(group.totalCogs)} ریال
                          </td>
                          <td className="p-4 font-mono font-bold text-emerald-700 bg-emerald-50/30">
                            {formatCurrency(group.totalProfit)} ریال
                          </td>
                          <td className="p-4 font-mono text-sky-700 font-bold">
                            {formatNumber(margin)}%
                          </td>
                        </tr>
                      );
              ...
              <div className="p-5 rounded-xl border border-gray-100 bg-gray-50 flex items-start gap-3">
                ...
                <div>
                  <h4 className="text-xs font-bold text-gray-800 mb-1">
                    مزیت گروه‌بندی ارزش افزوده کالاها
                  </h4>
                  <p className="text-[11px] text-gray-500 leading-relaxed">
                    شما می‌توانید فایلهایی با نرخ‌های مالیاتی متفاوت (کالاهای
                    معاف، لبنیات با عوارض کم، و کالاهای کالابرگی لوکس با مالیات
                    استاندارد ۱۰٪) را به تفکیک یا مجموع پردازش کنید و اظهارنامه
                    دوره‌ای را مطابقت دهید.
                  </p>
                </div>
              </div>
              <div className="p-5 rounded-xl border border-gray-100 bg-gray-50 flex items-start gap-3">
                ...
                <div>
                  <h4 className="text-xs font-bold text-gray-800 mb-1">
                    تطبیق بها با فیلتر تفصیل
                  </h4>
                  <p className="text-[11px] text-gray-500 leading-relaxed">
                    فعال کردن فیلتر تفصیل در بالای همین صفحه نیز تمام سودها و
                    تراکنش‌های عوارض این جدول را متناسب با خریدار منتخب محدود
                    می‌سازد تا گزارش مشتری کاملی داشته باشید.
                  </p>
                </div>
              </div>
            </div>
        ...
        {/* TAB 6: GLOBAL KARDEX */}
        {activeTab === "GLOBAL_KARDEX" && (
          <GlobalKardex kardexByItem={kardexByItem} />
        )}
      </div>
    </div>
  );
...
function SummaryCard({
  ...
  return (
    <div
      className={`p-6 rounded-2xl border flex flex-col gap-2.5 transition-all shadow-sm ${highlight ? "border-emerald-250 bg-emerald-50/20" : "border-gray-150 bg-gray-50/40 hover:bg-gray-50"}`}
    >
      ...
      <div className="flex flex-col mt-1 gap-1">
        ...
        <span className="text-base font-black font-mono text-gray-850">
          {formatNumber(qty)}
        </span>
      </div>
      <div className="flex flex-col mt-1 gap-1">
        ...
        <span className="text-lg font-black font-mono tracking-tight text-gray-900">
          ...
          <span className="text-[10px] font-sans font-normal text-gray-500 mr-1">
            ریال
          </span>
        </span>
      </div>
